...
{% macro maxcompute__test_expect_column_values_to_be_within_n_moving_stdevs(model,
                                  column_name,
                                  date_column_name,
                                  group_by,
                                  period,
...
from
    metric_sigma
where
    cast(metric_period as {{ dbt_expectations.type_timestamp() }}) >= cast(
            {{ dbt.dateadd(period, -test_periods, dbt.date_trunc(period, dbt_date.now())) }}
            as {{ dbt_expectations.type_timestamp() }})
    and
    cast(metric_period as {{ dbt_expectations.type_timestamp() }}) < {{ dbt.date_trunc(period, dbt_date.now()) }}
    and
    not (
        metric_test_sigma >= {{ sigma_threshold_lower }} and
        metric_test_sigma <= {{ sigma_threshold_upper }}
